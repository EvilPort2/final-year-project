...
def createDataset():
    ...
    conn = sqlite3.connect("face_recognition/faceDetectDatabase.db")
    try:
    ...
    if flag == 0:
        ...
        if choice == 1:
            ...
            for f in glob.glob("face_recognition/dataset/user." + str(id) + "*.jpg"):
                print(f)
                os.remove(f)
    ...
    while cam.isOpened():
        ...
        for (x, y, w, h) in faces:
            ...
            if len(eyes) == 2:
                cv2.imwrite("face_recognition/dataset/user." + str(id) + "." + str(i) + ".jpg", gray[y:y + h, x:x + w])
